...
function setpdttime(hObject, handles)
...
if ~isempty(answer)
    ...
    set(hObject,'checked','on');  % turn on the current checkmark
    handles.lp.plotscale=[ustr2num(answer{1}),str2num(answer{2}),str2num(answer{3}),str2num(answer{4})];
end
guidata(hObject,handles);
...
function setpsdfftsize(hObject, handles)
...
function setpsdwindow(hObject, handles)
...
function setpsdoverlap(hObject,handles)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.psd.overlap=0.50;  % set overlap to 50%
setpsdoverlap(hObject,handles)
...
function setspecdt(hObject, handles)
